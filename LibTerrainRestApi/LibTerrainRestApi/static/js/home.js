...
function initialize() {
    ...
    elevation.loadData(opts.elevationControl.data);
    drawEllipse();
}
...
function drawEllipse() {
    var coordinates = JSON.parse(geojson).features[0].geometry.coordinates;
    ...
    //punti partenza
    var startX = 0;
    var startY = elevation._y(firstH + 4);//altezza primo elemento + offset antenna
    var endX = elevation._width();
    var endY = elevation._y(lastH + 5);//altezza ultimo elemento + offset antenna
    ...
    var h = mY - startY;
    var angle = h / length * 180 / Math.PI;//(mX - startX) / h;
    //Draw the Ellipse
